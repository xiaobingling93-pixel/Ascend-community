...
出现该标签表示该PR中所包含的commit中，有部分贡献者没有签署Ascend社区的贡献者协议CLA，签署地址可以在PR评论区找到；
...
签署后会收到主题是Signing CLA on project of xx的邮件，请联系邮件内容里的Corporation Managers进行审批，审批通过后，在PR评论区评论/check-cla，重新触发CLA检查就会打上ascend-cla/yes标签。CLA检查是使用commit信息中的committer邮箱作为检查凭证的。该邮箱可以通过git log --pretty=fuller查询到。
...
## 4.  **请问社区的开发者角色可以直接push代码到仓库吗？**

开发者不允许直接push代码到社区仓库，只有仓库管理员可以push代码到社区仓库；
如果开发者想要贡献代码，只能fork社区仓库到个人账号下，通过提交Pull Request方式贡献代码。
